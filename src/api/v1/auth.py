...
ALLOWED_ROLES = [1, 7, 24]  # IDs de cargo permitidos para acciones administrativas
...
async def signup(payload: SignupRequest, req: Request, authorization: str | None = Header(default=None)):
    """
    Endpoint de registro. Protegido. Solo usuarios con cargo 1, 7 o 24 pueden crear otros usuarios.
    """
    # Validar JWT del usuario que intenta crear otro usuario
    if not authorization:
        raise HTTPException(status_code=401, detail="Falta header Authorization")
    ...
    try:
        token_payload = decode_access_token(token, jwt_secret)
        creator_cargo = token_payload.get("cargo")
    except Exception:
        raise HTTPException(status_code=401, detail="Token inválido o expirado")

    # Validar que el usuario tiene uno de los cargos permitidos
    if creator_cargo not in ALLOWED_ROLES:
        raise HTTPException(
            status_code=403,
            detail=f"No tiene permisos. Solo usuarios con cargo {ALLOWED_ROLES} pueden crear otros usuarios."
        )
...
async def get_cargos(req: Request, authorization: str | None = Header(default=None)):
    """
